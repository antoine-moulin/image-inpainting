...
### CLASS IMPORTS ###
import Image as InpaintingImage # to avoid confusion with other python build-in librairies
...
class Test :
    ...
    def __init__(self, test_number, description, image_parent_directory, fixed_parameters, variable_parameter):
        """ description of parameters :
            - global_test_number : number of this test (so it appears in the title of the file where the test object will be saved )
            ...
            --> scheme :  {'image':[[...]], 'mask':[[...]], 'is_color_image':True, 'patch_size':7, 'computation_patch_size':7}
        ...
        if type(variable_parameter["values"]) == list :
            ...
            raise Exception("variable_parameter['values'] is a ", type(variable_parameter["values"], ", however it should be a list."))
        ...
        if (self.variable_parameter_name == "image"):
            ...
            raise Exception("The dictionnary of variable parameter contains an unknown parameter name : ", self.variable_parameter_name)
        ...
        if (len(fixed_parameters_keys) != 6):
            raise Exception("There should be exactly 6 fixed parameters and 1 variable parameter. Current number of fixed parameters : ", len(fixed_parameters_keys))
        ...
        for fixed_parameter in fixed_parameters_keys :
            ...
            if (fixed_parameter == "image"):
                ...
                self.computation_patch_size_values = [fixed_parameters["computation_patch_size"]]*(self.number_of_tests)
            elif (fixed_parameter == "which_clustering_method"):
                self.which_clustering_method_values = [fixed_parameters["which_clustering_method"]]*(self.number_of_tests)
                ...
                raise Exception("The dictionnary of fixed parameters contains an unknown parameter name : ", fixed_parameter)
    ...
    def start_inpainting_test(self):
        ...
        for i in range(self.number_of_tests) :
            ...
            inpainting_image = InpaintingImage.Image(image, mask, is_color_image, patch_size, computation_patch_size, which_clustering_method, number_of_clusters, True)
